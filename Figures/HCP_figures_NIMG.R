...
names(cond.nonad) <- c("ind","mcp","predicted","power","true","simulation","condition")
...
pdf(paste(FIGDIR,"FIG_HCP_modelestimation.pdf",sep=""),width=9,height=5)
...
for (method in methods){
  ...
  effectorder <- order(res.nonad.mn$esc)[-1]

  powpred3D <- t(array(pre.nonad.mn[pre.nonad.mn$mcp==method,]$TPR,dim=c(47,50)))
  mm <- ifelse(method=="RF","RFT",method)
  powtrue3D <- t(array(obs.nonad.mn[obs.nonad.mn$mcp==method,]$TPR,dim=c(47,50)))
