...
estimation <- array(NA,dim=c(sims,16,6))
range <- (1:sims)
for (p in range){
...
powtru <- array(NA,dim=c(sims,16,20,4))
powpre <- array(NA,dim=c(sims,16,20,4))
for (p in 1:sims){
  for(c in 1:16){
...
legend(1,3,names(table(cons$acts)),fill=cols.d[9:12],border=cols.d[9:12],bty="n",title="% of brain active")
legend(1.5,3,names(table(cons$effs)),fill=Greys,border=Greys,bty="n",title="Effect size")

dev.off()

################################################

powpred <- array(NA,dim=c(sims,4,4,20,4)) 
powtrue <- array(NA,dim=c(sims,4,4,20,4)) 

colsmatrix <- matrix(c(Greens,Blues,Greys,Reds),nrow=4,byrow=TRUE)

for(s in (1:sims)[-c(134,225,456,548,623)]){
  print(s)
  for(p in 1:4){
    for(e in 1:4){
      per <- c(2,4,6,8)[p]
...
t3 <- levelplot(t(bias[,,method]),
                ...
                ylab="",
                aspect="fill")
t1t2 <- print(c(t1,t2,layout=c(2,1),merge.legends=TRUE,x.same=TRUE,y.same=TRUE))
