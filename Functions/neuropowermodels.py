...
def altPDF(peaks,mu,sigma,exc=None,method="RFT"):
	"""
...
def altCDF(peaks,mu,sigma,exc=None,method="RFT"):
...
def mixPDF(peaks,pi1,mu,sigma,exc=None,method="RFT"):
...
def mixPDF_SLL(pars,peaks,pi1,exc=None,method="RFT"):
	...
	if method == "RFT":
		...
		f = mixPDF(peaks,pi1=pi1,mu=mu,sigma=sigma,method="CS")
	LL = -sum(np.log(f))
	return(LL)
...
def modelfit(peaks,pi1,exc=None,starts=1,seed=None,method="RFT"):
	...
	seed = np.random.uniform(0,1000,1) if not 'seed' in locals() else seed
	sigmas = np.random.uniform(0.1,10,(starts,)) if method=="RFT" else np.random.uniform(0,10,(starts,))
	best = []
	par = []
