...
for c in range(47):
    ...
    maskfile = os.path.join(HOMEDIR,'mask_HCP.nii.gz')
    ...
    fslcmd = 'flameo --copefile=true_cope.nii.gz --covsplitfile=design.grp --designfile=design.mat --ld=stats --maskfile=%s --runmode=ols --tcontrastsfile=design.con' %(os.path.join(HOMEDIR,'mask_HCP.nii.gz'))
    ...
    fslcmd = 'flameo --copefile=cope.nii.gz --covsplitfile=design.grp --designfile=design.mat --ld=stats --maskfile=%s --runmode=ols --tcontrastsfile=design.con' %(os.path.join(HOMEDIR,'mask_HCP.nii.gz'))
    os.popen(fslcmd).read()
